...
if (!function_exists('verifyCardCredentials')) {
    ...
    function verifyCardCredentials(PDO $pdo, string $code, string $cvv, string $ip): array {
        ...
        try {
            $rl = $pdo->prepare("SELECT COUNT(*) FROM card_verify_attempts
                                  WHERE ip = :ip AND success = 0
                                    AND created_at > (NOW() - INTERVAL 1 HOUR)");
            $rl->execute([':ip' => $ip]);
            if ((int) $rl->fetchColumn() >= 5) {
                return ['ok' => false, 'error' => 'धेरै पटक गलत प्रयास भयो। 1 घण्टा पछि पुनः प्रयास गर्नुहोस्।', 'rate_limited' => true];
            }
