...
if ($action === 'list' && $tableExists) {
    try {
        /* counts */
        $totalCount    = (int)$db->query("SELECT COUNT(*)    FROM chatbot_faqs")->fetchColumn();
        $activeCount   = (int)$db->query("SELECT COUNT(*)    FROM chatbot_faqs WHERE is_active=1")->fetchColumn();
