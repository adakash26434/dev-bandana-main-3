...
<div class="mp-container">
  ...
  <div class="wf-pane <?= $activeTab==='new'?'active':'' ?>" id="acc-pane-new">
    ...
    <form method="POST">
      ...
      <!-- Nominee -->
      <div style="font-size:.82rem;font-weight:700;color:var(--text-primary,#1a2e1f);margin:16px 0 8px;padding-top:12px;border-top:1px solid var(--border-color,#e5e7eb);">
        <i class="fas fa-user-friends" style="color:var(--primary-color);margin-right:6px;"></i><?php echo $_t('नमिनी विवरण (Optional)', 'Nominee Details (Optional)'); ?>
      </div>
      <div class="mem-form-row mem-form-row-2">
        <div class="mem-form-group">
  ...
  <div class="wf-pane <?= $activeTab==='history'?'active':'' ?>" id="acc-pane-history">
    ...
    <div style="padding:40px 0;text-align:center;">
      <i class="fas fa-piggy-bank" style="font-size:2rem;color:var(--text-muted,#9ca3af);display:block;margin-bottom:10px;"></i>
      ...
      <div style="font-size:.82rem;color:var(--text-muted,#6b7280);"><?php echo $_t('"नयाँ आवेदन" tab बाट खाता खोल्नुहोस्।', 'Use "New Application" tab to apply.'); ?></div>
    </div>
    <?php else: ?>
    ...
    <div class="recent-card">
      <div>
        <div style="font-size:.87rem;font-weight:600;color:var(--text-primary,#1a2e1f)"><?= $acLabel ?> — रु. <?= number_format((float)$ac['initial_deposit']) ?></div>
        <div style="font-size:.75rem;color:var(--text-muted,#6b7280);margin-top:3px"><?= date('Y-m-d', strtotime($ac['created_at'])) ?></div>
      </div>
