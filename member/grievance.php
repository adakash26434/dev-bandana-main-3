...
<div class="mp-container">
  ...
  <div class="wf-pane <?= $activeTab==='new'?'active':'' ?>" id="grv-pane-new">
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="mem-form-row mem-form-row-2">
        ...
        <div class="mem-form-group" style="display:flex;align-items:flex-end;padding-bottom:14px;">
          <label style="display:flex;align-items:center;gap:8px;font-size:.85rem;font-weight:600;color:var(--text-secondary);cursor:pointer;">
            <input type="checkbox" name="is_anonymous" value="1" <?= isset($_POST['is_anonymous']) ? 'checked' : '' ?> style="width:16px;height:16px;accent-color:var(--primary-color);">
      ...
      <div class="mem-form-group">
        <label class="mem-form-label"><i class="fas fa-paperclip" style="color:var(--primary-color);margin-right:4px;"></i><?php echo $_t('संलग्न फाइल (Optional)', 'Attachment (Optional)'); ?></label>
        <input type="file" name="attachment" class="mem-form-control" accept=".jpg,.jpeg,.png,.pdf,.doc,.docx">
        <div style="font-size:.75rem;color:var(--text-muted);margin-top:4px;"><?php echo $_t('JPG, PNG, PDF, DOC — अधिकतम 5MB', 'JPG, PNG, PDF, DOC — max 5MB'); ?></div>
      </div>
  ...
  <div class="wf-pane <?= $activeTab==='history'?'active':'' ?>" id="grv-pane-history">
    ...
    <div style="padding:40px 0;text-align:center;">
      <i class="fas fa-comment-slash" style="font-size:2rem;color:var(--text-muted,#9ca3af);display:block;margin-bottom:10px;"></i>
      <div style="font-weight:600;color:var(--text-primary,#1a2e1f);margin-bottom:4px;"><?php echo $_t('कुनै गुनासो छैन', 'No grievances yet'); ?></div>
      <div style="font-size:.82rem;color:var(--text-muted,#6b7280);"><?php echo $_t('"नयाँ गुनासो" tab बाट गुनासो दर्ता गर्नुहोस्।', 'Use "New Grievance" tab to file.'); ?></div>
    </div>
    <?php else: ?>
    ...
    <div class="recent-card">
      <div style="flex:1;min-width:0;">
        <div style="font-size:.87rem;font-weight:600;color:var(--text-primary,#1a2e1f);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($gr['subject']) ?></div>
        <div style="font-size:.75rem;color:var(--text-muted,#6b7280);margin-top:3px;">
          <?= $catLabel ?>
      ...
      <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;flex-shrink:0;">
        ...
        <span class="sr-status <?= $stCls ?>" style="font-size:.75rem;font-weight:700;"><?= $stLbl ?></span>
      </div>
    </div>
    <?php endforeach; ?>
    <a href="tracker.php" style="font-size:.82rem;color:var(--primary-color);text-decoration:none;font-weight:600;display:inline-block;margin-top:8px;">
