...
$loanRates = [];
try {
    $loanRates = $db->query("SELECT * FROM interest_rates WHERE type='loan' AND is_active=1 ORDER BY display_order ASC LIMIT 15")->fetchAll(PDO::FETCH_ASSOC);
} catch (Throwable $e) {}
...
<div class="mp-container">
  ...
  <div class="wf-pane <?= $activeTab==='new'?'active':'' ?>" id="loan-pane-new">
    ...
    <form method="POST">
      ...
      <div style="font-size:.8rem;color:var(--text-muted,#6b7280);margin-bottom:12px;">
        <i class="fas fa-calculator" style="color:var(--primary-color);margin-right:4px;"></i>
        <?php echo $_t('मासिक किस्ता अनुमान:', 'Estimate monthly installment:'); ?>
        <a href="<?= SITE_URL ?>member/apply-frame.php?p=emi" style="color:var(--primary-color);font-weight:600;text-decoration:none;">EMI Calculator →</a>
      </div>
  ...
  <div class="wf-pane <?= $activeTab==='history'?'active':'' ?>" id="loan-pane-history">
    ...
    <div style="padding:40px 0;text-align:center;">
      <i class="fas fa-file-circle-xmark" style="font-size:2rem;color:var(--text-muted,#9ca3af);display:block;margin-bottom:10px;"></i>
      <div style="font-weight:600;color:var(--text-primary,#1a2e1f);margin-bottom:4px;"><?php echo $_t('कुनै आवेदन छैन', 'No applications yet'); ?></div>
      <div style="font-size:.82rem;color:var(--text-muted,#6b7280);"><?php echo $_t('"नयाँ आवेदन" tab बाट ऋणको लागि आवेदन दिनुहोस्।', 'Use "New Application" tab to apply.'); ?></div>
    </div>
    <?php else: ?>
    ...
    <div class="recent-card">
      <div>
        <div style="font-size:.87rem;font-weight:600;color:var(--text-primary,#1a2e1f)"><?= htmlspecialchars($ln['loan_type']) ?> — रु. <?= number_format((float)$ln['loan_amount']) ?></div>
        <div style="font-size:.75rem;color:var(--text-muted,#6b7280);margin-top:3px"><?= date('Y-m-d', strtotime($ln['created_at'])) ?></div>
      </div>
