...
<div class="mp-container">
  ...
  <div class="wf-pane <?= $activeTab==='new'?'active':'' ?>" id="ds-pane-new">
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="mem-form-group">
        ...
        <div style="font-size:.75rem;color:var(--text-muted);margin-top:4px;"><?php echo $_t('JPG, PNG, PDF — अधिकतम 5MB', 'JPG, PNG, PDF — max 5MB'); ?></div>
      </div>
  ...
  <div class="wf-pane <?= $activeTab==='history'?'active':'' ?>" id="ds-pane-history">
    ...
    <div style="padding:40px 0;text-align:center;">
      <i class="fas fa-laptop-code" style="font-size:2rem;color:var(--text-muted,#9ca3af);display:block;margin-bottom:10px;"></i>
      <div style="font-weight:600;color:var(--text-primary,#1a2e1f);margin-bottom:4px;"><?php echo $_t('कुनै अनुरोध छैन', 'No requests yet'); ?></div>
      <div style="font-size:.82rem;color:var(--text-muted,#6b7280);"><?php echo $_t('"नयाँ अनुरोध" tab बाट सेवा माग्नुहोस्।', 'Use "New Request" tab to submit.'); ?></div>
    </div>
    <?php else: ?>
    ...
    <div class="recent-card">
      <div>
        <div style="font-size:.87rem;font-weight:600;color:var(--text-primary,#1a2e1f)"><?= $svcLabel ?></div>
        <div style="font-size:.75rem;color:var(--text-muted,#6b7280);margin-top:3px"><?= date('Y-m-d', strtotime($req['created_at'])) ?></div>
      </div>
