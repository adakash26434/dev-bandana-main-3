            ...
            if (empty($full_name)) {
                ...
                foreach ($newCols as $col => $def) {
                    if (function_exists('safeAddColumn')) {
                        ...
                        try {
                            $c = $db->query("SHOW COLUMNS FROM kyc_applications LIKE '$col'");
                            if (!$c || $c->fetch() === false) {
                        ...
                        try {
                            ...
                            if (getSetting('notify_sms_enabled', '0') === '1' && $smsToken) {
                                ...
                                if (strlen($ph) >= 10) {
                                    ...
                                    curl_setopt_array($ch, [CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>http_build_query(['token'=>$smsToken,'from'=>$smsSender,'to'=>$ph,'text'=>mb_substr($smsTxt,0,160)]),CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>10,CURLOPT_SSL_VERIFYPEER=>false]);
            ...
            <div class="col-lg-10">
                <div class="kyc-form-box" data-aos="fade-up">
                    ...
                    <form method="POST" enctype="multipart/form-data" class="kyc-form needs-validation" id="fullKymForm" novalidate>
                        <?php echo csrfField(); ?>
                        <div id="kymWizardNav" class="kym-wizard-nav mb-3"></div>
...
(function () {
    ...
    function getSectionTitle(sec, idx) {
        ...
        if (!t) t = (<?php echo json_encode(isEnglish() ? 'Section' : 'भाग'); ?>) + ' ' + (idx + 1);
    ...
    function renderNav() {
        ...
        sections.forEach(function (sec, idx) {
            ...
            b.type = 'button';
            b.className = 'kym-step-btn' + (idx === currentStep ? ' active' : '');
            b.textContent = (idx + 1) + '. ' + getSectionTitle(sec, idx);
            b.addEventListener('click', function () {
                saveDraft();
                setStep(idx);
            });
    ...
    function setStep(idx) {
        ...
        sections.forEach(function (sec, sidx) { sec.style.display = (sidx === currentStep) ? '' : 'none'; });
    ...
    if (nextBtn) nextBtn.addEventListener('click', function () { saveDraft(); setStep(currentStep + 1); });
    if (prevBtn) prevBtn.addEventListener('click', function () { saveDraft(); setStep(currentStep - 1); });
...
.kyc-form-section .form-control.form-control-sm,
.kyc-form-section .form-select.form-select-sm {
    min-height: 34px;
    padding: 0.3rem 0.58rem;
    font-size: 0.84rem;
    border-radius: 8px;
}
.kyc-form-section .input-group > .form-control,
.kyc-form-section .input-group > .form-select {
    min-height: 40px;
}
.kyc-form-section .input-group-text {
    min-height: 40px;
    padding: 0.4rem 0.72rem;
    border-radius: 10px;
}
.kyc-form-section textarea.form-control {
    min-height: 84px;
}
...
.kym-subsec-title.kym-subsec-open {
    background: #dbeafe;
}
.kym-acc-badge {
    margin-left: 8px;
...
@media (max-width: 768px) {
    ...
    #publicQuickKycPanel .form-section {
        padding: 12px;
    }
    #kymWizardControls {
        gap: 8px;
    }
    #kymWizardControls .btn {
